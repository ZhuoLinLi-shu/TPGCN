...
class gcn_personal(nn.Module):
    def __init__(self, c_in, c_out, dropout, support_len=3, order=2, time_length=0):
        ...
        self.query_2 = nn.Conv2d(in_channels=c_out, out_channels=1, kernel_size=(1, 12), bias=True) # time_length

        for i in range(self.order):
            self.mlp_project.append(linear(c_out, c_out)) # c_out
            self.mlp_project_1.append(nn.Conv2d(c_out, 1, kernel_size=(1, 1), bias=True)) # time_length
    ...
    def forward(self, x, support, mlp_part, pred_time_embed):
        ...
        x1, attn = self.personal_atten(x1_driving, mlp_part, query, layer=0)
        attention_out = [attn]

        out.append(x1)
        for k in range(2, self.order + 1):
            x2_driving = self.nconv(x1, support)
            x2, attn_ = self.personal_atten(x2_driving, mlp_part, query, layer=k-1)
            attention_out.append(attn_)
            out.append(x2)
            x1 = x2
        ...
        attention_out_ = torch.cat(attention_out, dim=1)
        return h, attention_out_
...
class TPGCN(nn.Module):
    ...
    def forward(self, input, pred_time_embed=None):
        ...
        if self.gcn_bool:
            adp, resolution_static, _, gl_loss_ = self.graph_construct(input, mlp_component)
            gl_loss = gl_loss_
            if self.addaptadj:
                new_supports = adp
        ...
        for i in range(self.layers):
            ...
            if self.gcn_bool:
                x, atten_out = self.gconv[i](x, new_supports, mlp_component, pred_time_embed)
            else:
