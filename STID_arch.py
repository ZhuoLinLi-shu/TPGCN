...
from TPGCN.mlp import MultiLayerPerceptron
    ...
    def __init__(self, num_nodes, input_len, output_len, num_layer, input_dim=3, node_dim=32, embed_dim=32,
        ...
        if self.if_time_in_day:
            self.time_in_day_emb = nn.Parameter(
                torch.empty(self.first_time, self.temp_dim_tid)) # 288
            nn.init.xavier_uniform_(self.time_in_day_emb)
        if self.if_day_in_week:
            self.day_in_week_emb = nn.Parameter(
                torch.empty(self.second_time, self.temp_dim_diw)) # 7
        ...
        if self.if_time_in_day:
            ...
            if self.time_norm:
                time_in_day_emb = self.time_in_day_emb[(
                    t_i_d_data[:, -1, :] * self.first_time).type(torch.LongTensor)]
            else:
